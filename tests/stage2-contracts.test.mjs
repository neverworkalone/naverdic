...
test('keeps the official Chrome Translator display name and translation panel boundary', () => {
  ...
  assert.match(toolbarPopup, /\.naverdic-popup-body--scrollable \{[\s\S]*max-height: 336px;[\s\S]*overflow-y: auto/)
  assert.match(toolbarPopup, /\.naverdic-popup-shell \.dictionary-result \{[\s\S]*width: 340px[\s\S]*height: auto;[\s\S]*max-height: none;[\s\S]*overflow: visible/)
