...
test('defines the v2 storage split and nested settings defaults', () => {
  ...
  assert.equal(SETTINGS_V2_DEFAULTS.recentSearch.enabled, true)
  ...
  assert.equal(createInitialSettingsV2().recentSearch.enabled, true)
...
test('normalizes v2 values without accepting invalid enums or provider secrets', () => {
  ...
  assert.equal(normalized.dictionary.drag.triggerKey, 'alt')
  ...
  assert.equal(normalized.recentSearch.enabled, true)
...
test('keeps the official Chrome Translator display name and translation panel boundary', () => {
  ...
  assert.match(toolbarPopup, /\.naverdic-popup-shell--recent \{[\s\S]*height: auto;[\s\S]*min-height: 0;/)
  ...
  assert.match(toolbarPopup, /\.naverdic-popup-body--result \{[\s\S]*max-height: 336px;[\s\S]*overflow-y: auto/)
  assert.match(toolbarPopup, /v-if="hasVisibleResult"/)
  assert.equal(toolbarPopup.includes('shouldUseScrollableResult'), false)
  ...
  assert.match(appearancePage, /\.settings-page\[data-page-id='translation-service'\] \{[\s\S]*margin-top: 0/)
...
test('falls back only damaged v6.6 values while preserving valid values', () => {
  ...
  assert.equal(result.settings.dictionary.drag.triggerKey, 'alt')
